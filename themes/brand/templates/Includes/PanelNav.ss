<aside class="panel panel--nav $NavigationWidth columns <% if $NavigationPosition =='With Content' %>panel-content<% end_if %>">
	<div class="panel-block type-nav $Top.Colour.LowerCase() first">
			...
			<% loop $Menu(1) %>
				<li class="<% if $FirstLast %>nav-$FirstLast<% end_if %> nav-$LinkingMode nav-level1 <% if $LinkingMode = current %> active<% end_if %> <% if $Link = home && $SiteConfig.Logo %> logo<% end_if %><% if $Children %> has-children<% end_if %>">
