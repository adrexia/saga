<aside class="menu menu-control" id="menu">
	...
	<% loop $Menu(1) %>
		<li class="<% if $FirstLast %>nav-$FirstLast<% end_if %> $LinkingMode menu-level1 <% if $LinkingMode = current %> active<% end_if %> <% if $Link = home && $SiteConfig.Logo %> logo<% end_if %><% if $Children %>has-children<% end_if %>">
